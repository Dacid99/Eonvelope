...
Fixtures:
    ...
    :func:`fixture_attachmentModel`: Creates an email in `accountModel`.
    :func:`fixture_attachmentModel`: Creates an attachment in `emailModel`.
    :func:`fixture_emailPayload`: Creates clean :class:`Emailkasten.Models.AttachmentModel.AttachmentModel` payload for a post or put request.
...
def fixture_attachmentPayload(emailModel) -> dict[str, Any]:
    """Creates clean :class:`Emailkasten.Models.AttachmentModel.AttachmentModel` payload for a post or put request.
...
def fixture_list_url() -> str:
    ...
    Returns:
        The list url.
    """
    return reverse(f'{AttachmentViewSet.BASENAME}-list')

@pytest.fixture(name='detail_url')
def fixture_detail_url(attachmentModel) -> str:
    """Gets the viewsets url for detail actions.

    Args:
        attachmentModel: Depends on :func:`fixture_attachmentModel`.

    Returns:
        The detail url."""
    return reverse(f'{AttachmentViewSet.BASENAME}-detail', args=[attachmentModel.id])
...
def fixture_custom_list_action_url() -> Callable[[str],str]:
    ...
    return lambda custom_list_action_url_name: (
        reverse(f'{AttachmentViewSet.BASENAME}-{custom_list_action_url_name}')
    )

@pytest.fixture(name='custom_detail_action_url')
def fixture_custom_detail_action_url(attachmentModel)-> Callable[[str],str]:
    ...
    return lambda custom_detail_action_url_name: (
        reverse(f'{AttachmentViewSet.BASENAME}-{custom_detail_action_url_name}', args=[attachmentModel.id])
    )
...
@pytest.mark.django_db
def test_patch_noauth(attachmentModel, noauth_apiClient, detail_url):
    ...
    assert attachmentModel.file_name != 'abc123'


@pytest.mark.django_db
def test_patch_auth_other(attachmentModel, other_apiClient, detail_url):
...
@pytest.mark.django_db
def test_patch_auth_owner(attachmentModel, owner_apiClient, detail_url):
...
def test_put_noauth(attachmentModel, noauth_apiClient, attachmentPayload, detail_url):
    ...
    response = noauth_apiClient.put(detail_url, data=attachmentPayload)
...
def test_put_auth_other(attachmentModel, other_apiClient, attachmentPayload, detail_url):
    ...
    response = other_apiClient.put(detail_url, data=attachmentPayload)
...
def test_put_auth_owner(attachmentModel, owner_apiClient, attachmentPayload, detail_url):
    ...
    response = owner_apiClient.put(detail_url, data=attachmentPayload)
...
def test_post_noauth(noauth_apiClient, attachmentPayload, list_url):
    ...
    response = noauth_apiClient.post(list_url, data=attachmentPayload)
...
def test_post_auth_other(other_apiClient, attachmentPayload, list_url):
    ...
    response = other_apiClient.post(list_url, data=attachmentPayload)
...
def test_post_auth_owner(owner_apiClient, attachmentPayload, list_url):
    ...
    response = owner_apiClient.post(list_url, data=attachmentPayload)
...
def test_toggle_favorite_noauth(attachmentModel, noauth_apiClient, custom_detail_action_url):
    ...
    with an unauthenticated user client.
    """
    response = noauth_apiClient.post(custom_detail_action_url(AttachmentViewSet.URL_NAME_TOGGLE_FAVORITE))
...
def test_toggle_favorite_auth_other(attachmentModel, other_apiClient, custom_detail_action_url):
    ...
    with the authenticated other user client.
    """
    response = other_apiClient.post(custom_detail_action_url(AttachmentViewSet.URL_NAME_TOGGLE_FAVORITE))
...
def test_toggle_favorite_auth_owner(attachmentModel, owner_apiClient, custom_detail_action_url):
    ...
    with the authenticated owner user client.
    """
    response = owner_apiClient.post(custom_detail_action_url(AttachmentViewSet.URL_NAME_TOGGLE_FAVORITE))
