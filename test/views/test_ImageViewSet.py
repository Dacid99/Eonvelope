...
"""Test module for :mod:`api.v1.views.ImageViewSet`.
...
def test_download_noauth(imageModel, noauth_apiClient, custom_detail_action_url, mocker):
    """Tests the get method :func:`api.v1.views.ImageViewSet.ImageViewSet.download` action
    with an unauthenticated user client.
    """
    mock_open = mocker.patch('api.v1.views.ImageViewSet.open')
    mock_os_path_exists = mocker.patch('api.v1.views.ImageViewSet.os.path.exists', return_value=True)
...
def test_download_no_file_auth_owner(imageModel, owner_apiClient, custom_detail_action_url, mocker):
    ...
    """
    mock_open = mocker.patch('api.v1.views.ImageViewSet.open')
    mock_os_path_exists = mocker.patch('api.v1.views.ImageViewSet.os.path.exists', return_value=False)

    response = owner_apiClient.get(custom_detail_action_url(ImageViewSet, ImageViewSet.URL_NAME_DOWNLOAD, imageModel))

    assert response.status_code == status.HTTP_404_NOT_FOUND
    mock_open.assert_not_called()
    mock_os_path_exists.assert_called_once_with(imageModel.file_path)
...
def test_download_auth_owner(imageModel, owner_apiClient, custom_detail_action_url, mocker):
    ...
    mocker.patch('api.v1.views.ImageViewSet.open', mock_open)
    mock_os_path_exists = mocker.patch('api.v1.views.ImageViewSet.os.path.exists', return_value=True)
...
def test_toggle_favorite_noauth(imageModel, noauth_apiClient, custom_detail_action_url):
    """Tests the post method :func:`api.v1.views.ImageViewSet.ImageViewSet.toggle_favorite` action
    with an unauthenticated user client.
    """
